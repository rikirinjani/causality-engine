...
var last_checkpoint_tick: int = -1

## P-015 instrumentation: per-request latency (ms) with URL, recorded in queue order
var request_latencies: Array = []
var _pending_ts: Array = []
var _inflight_url: String = ""
...
func _queue_request(url: String, headers: PackedStringArray = PackedStringArray(), method: int = HTTPClient.METHOD_GET, body: String = "") -> void:
    _pending_ts.append(Time.get_ticks_msec())
...
func _on_http_completed(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray) -> void:
    if _pending_ts.size() > 0:
        var start_ts: int = _pending_ts.pop_front()
        request_latencies.append({"url": _inflight_url, "ms": float(Time.get_ticks_msec() - start_ts), "code": response_code})
    ...
    if data_dict.has("checkpoint"):
        last_checkpoint = data_dict["checkpoint"] as String
        last_delivery = data_dict["delivery"] as String
        last_checkpoint_tick = ce_tick
...
func _process_next_request() -> void:
    if pending_requests.size() > 0:
        ...
        _inflight_url = next["url"] as String
...
func _project_state(data: Dictionary) -> void:
    ...
    for town_id in regions:
        ...
        towns[town_id] = {
            ...
            "patrol_demand": region.get("patrolDemand", region.get("patrol_demand", 0.0)),
